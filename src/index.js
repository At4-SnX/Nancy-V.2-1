...
async function execute(ctx, command, args, slash = false) {
  ...
  if (command === 'antibot') { const state = (slash ? ctx.options.getString('etat') : args[0])?.toLowerCase(); if (!['on', 'off'].includes(state)) return reply(ctx, 'Usage : antibot on/off', true); config(guild.id).antibot = state === 'on'; save(); return reply(ctx, `Antibot **${state === 'on' ? 'activé' : 'désactivé'}**.`); }
  if (command === 'antinuke') { const state = (slash ? ctx.options.getString('etat') : args[0])?.toLowerCase(); const threshold = slash ? ctx.options.getInteger('seuil') : Number(args[1] || 3); const action = (slash ? ctx.options.getString('action') : args[2] || 'strip').toLowerCase(); if (!['on', 'off'].includes(state) || !Number.isInteger(threshold) || threshold < 2 || threshold > 20 || !['strip', 'kick', 'ban'].includes(action)) return reply(ctx, 'Usage : antinuke on/off [seuil 2-20] [strip|kick|ban]', true); config(guild.id).antinuke = { enabled: state === 'on', threshold, action }; save(); return reply(ctx, `Antinuke **${state === 'on' ? 'activé' : 'désactivé'}** (seuil ${threshold}, action ${action}).`); }
}
