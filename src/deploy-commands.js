...
const commands = [
  ...
  new SlashCommandBuilder().setName('antispam').setDescription('Configurer la protection antispam').addStringOption(o => o.setName('etat').setDescription('on ou off').setRequired(true).addChoices({ name: 'on', value: 'on' }, { name: 'off', value: 'off' })).addIntegerOption(o => o.setName('messages').setDescription('Messages autorisés dans la fenêtre (défaut : 6)').setMinValue(3).setMaxValue(15)).addIntegerOption(o => o.setName('secondes').setDescription('Taille de la fenêtre en secondes (défaut : 7)').setMinValue(2).setMaxValue(60)).addStringOption(o => o.setName('timeout').setDescription('Durée de sanction, ex. 10m (défaut : 10m)')),
  ...
  new SlashCommandBuilder().setName('coins').setDescription('Voir vos Nancy Coins').addUserOption(o => o.setName('membre').setDescription('Membre à consulter')),
  new SlashCommandBuilder().setName('shop').setDescription('Voir la boutique des rôles'),
  new SlashCommandBuilder().setName('buy').setDescription('Acheter un rôle de la boutique').addRoleOption(o => o.setName('role').setDescription('Rôle à acheter').setRequired(true)),
  new SlashCommandBuilder().setName('shoprole').setDescription('Ajouter ou mettre à jour un rôle de boutique').addRoleOption(o => o.setName('role').setDescription('Rôle à vendre').setRequired(true)).addIntegerOption(o => o.setName('prix').setDescription('Prix en Nancy Coins').setMinValue(1).setRequired(true)),
  new SlashCommandBuilder().setName('statistique').setDescription('Voir les statistiques d’activité du serveur'),
  new SlashCommandBuilder().setName('giveawaymanager').setDescription('Définir le rôle gérant les giveaways').addRoleOption(o => o.setName('role').setDescription('Rôle autorisé à lancer un giveaway').setRequired(true)),
  new SlashCommandBuilder().setName('giveaway').setDescription('Créer un giveaway').addStringOption(o => o.setName('prix').setDescription('Lot à remporter').setRequired(true)).addStringOption(o => o.setName('duree').setDescription('Ex. 10m, 2h, 1d').setRequired(true)).addIntegerOption(o => o.setName('gagnants').setDescription('Nombre de gagnants').setMinValue(1).setMaxValue(20).setRequired(true)).addStringOption(o => o.setName('image').setDescription('URL GIF/image facultative')),
  new SlashCommandBuilder().setName('reroll').setDescription('Tirer de nouveaux gagnants').addStringOption(o => o.setName('id').setDescription('ID du giveaway terminé').setRequired(true)),
